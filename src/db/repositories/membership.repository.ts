...
export async function findProjectRole(userId: string, projectId: string): Promise<ProjectRole | undefined> {
...
export async function listProjectMemberIds(projectId: string): Promise<string[]> {
  ...
  return result.rows.map((membership: { user_id: string }) => membership.user_id);
}

export async function addProjectMember(projectId: string, userId: string, projectRole: ProjectRole): Promise<void> {
  await pool.query(`
