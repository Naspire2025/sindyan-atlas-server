...
export async function updateBudgetLineController(request: Request, response: Response, next: NextFunction): Promise<void> {
  try { response.json(await updateBudgetLineRecord(requireUser(request.user), parseUuid(request.params.id), request.body)); } catch (error) { next(error); }
}

export async function deleteBudgetLineController(request: Request, response: Response, next: NextFunction): Promise<void> {
  try { await deleteBudgetLineRecord(requireUser(request.user), parseUuid(request.params.id)); response.status(204).send(); } catch (error) { next(error); }
}
...
export async function updateSpendRecordController(request: Request, response: Response, next: NextFunction): Promise<void> {
  try { response.json(await updateSpendRecordRecord(requireUser(request.user), parseUuid(request.params.id), request.body)); } catch (error) { next(error); }
}

export async function deleteSpendRecordController(request: Request, response: Response, next: NextFunction): Promise<void> {
  try { await deleteSpendRecordRecord(requireUser(request.user), parseUuid(request.params.id)); response.status(204).send(); } catch (error) { next(error); }
}
