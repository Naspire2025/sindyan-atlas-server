...
export async function getProjectRole(user: AuthenticatedUser, projectId: number): Promise<ProjectRole | 'admin' | undefined> {
  if (user.role === 'admin') return 'admin';
  return findProjectRole(user.id, projectId);
}

export async function requireProjectAccess(user: AuthenticatedUser, projectId: number): Promise<ProjectRole | 'admin'> {
  const role = await getProjectRole(user, projectId);
  if (!role) throw new AppError(404, 'Project unavailable.');
  return role;
}

export async function requireProjectLead(user: AuthenticatedUser, projectId: number): Promise<void> {
